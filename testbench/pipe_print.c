...
// If REG_RANGE is 1 will only print registers in the cycle range specified.
#define REG_RANGE 0
#define REG_RANGE_MIN 30
#define REG_RANGE_MAX 50
...
void print_new_line_reg()
{
  fprintf(comfile, "");
}

void check_reset(int r)
{
	reset=r;
}
...
{
	if(!REG_RANGE ||((cycle_count>=REG_RANGE_MIN) && (cycle_count <=REG_RANGE_MAX)))
	{
    	if (regfile == NULL)
	       regfile = fopen("reg.out", "w");
		fprintf(regfile,"\n---Cycle %d\n",cycle_count);
	}
}

void print_rege(int i,int p1,int p0)
{
  long tmp=(long)p0 | (long)p1 <<32;
  if(!REG_RANGE ||((cycle_count>=REG_RANGE_MIN) && (cycle_count <=REG_RANGE_MAX)))
  {
    if(!reset)
    {
...
void print_commit(int wb_reg_wr_data_out_hi, int wb_reg_wr_data_out_lo,
                  int wb_reg_wr_idx_out, int wb_reg_wr_en_out)
{
  if (comfile == NULL)
       comfile = fopen("reg_commit.out", "w");

  if(wb_reg_wr_en_out)
    if((wb_reg_wr_data_out_hi==0)||
       ((wb_reg_wr_data_out_hi==-1)&&(wb_reg_wr_data_out_lo<0)))
      fprintf(comfile, "r%d=%x\n",wb_reg_wr_idx_out,wb_reg_wr_data_out_lo);
    else 
      fprintf(comfile, "r%d=0x%x%x\n",wb_reg_wr_idx_out,
              wb_reg_wr_data_out_hi,wb_reg_wr_data_out_lo);
