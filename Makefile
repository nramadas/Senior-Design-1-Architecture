...
clean:
	...
	rm -rf synsimv synsimv.daidir csrc vcdplus.vpd vcs.key synprog.out writeback.out vc_hdrs.h rs_ex_nosyn.out
	rm -rf rs.out pipeline.out ROB.out reg.out reg_commit.out rs_ex.out
	rm -rf *~ verilog/*~ testbench/*~
